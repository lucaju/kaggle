...
export const collectCompetitions = async (page) => {
	...
	try {
		...
		//get list of items
		const list = await page.$$('#site-content > div:nth-child(2) > div > div:nth-child(3) > div:nth-child(3) > ul > li:nth-child(odd)');
...
const getDetails = async (item) => {
	...
	try {
		...
		const title = await item.$eval('a > span:nth-child(2) > div:first-child', content => content.innerText);
		console.log(`:: ${title}`);

		const endpoint = await item.$eval('a', content => content.getAttribute('href'));
		const shortDescription = await item.$eval('a > span:nth-child(2) > span', content => content.innerText);
		
		const metadata = {};
		let meta = await item.$eval('a > span:nth-child(2) > span:nth-child(3)', content => content.innerText);
		if (meta) {
			meta = meta.trim();
			meta = meta.split('•');
