...
const getName = async (item) => {
  const name = await item
    .$eval('div:nth-child(2) > div:nth-child(4) > p:nth-child(1)', (content) => content.innerText)
    .catch((error) => processError(error));
  return name;
};
...
const getJoinedAt = async (item) => {
  const joinedAt = await item
    .$eval('div:nth-child(2) > div:nth-child(4) > p:nth-child(2) > span', (content) => content.getAttribute('title'))
    .catch((error) => processError(error));
...
const getTier = async (item) => {
  const tier = await item
    .$eval('div:nth-child(2) > div:nth-child(2) > img', (content) => content.getAttribute('title'))
    .catch((error) => processError(error));
  return tier;
};
...
const getGoldMedal = async (item) => {
  const medal = await item
    .$eval('div:nth-child(2) > div:nth-child(5) > div:nth-child(1) > div:nth-child(2)', (content) =>
...
const getSilverMedal = async (item) => {
  const medal = await item
    .$eval('div:nth-child(2) > div:nth-child(5) > div:nth-child(2) > div:nth-child(2)', (content) =>
      Number(content.innerText)
    )
    .catch((error) => processError(error));
  return medal;
...
const getBronzeMedal = async (item) => {
  const medal = await item
    .$eval('div:nth-child(2) > div:nth-child(5) > div:nth-child(3) > div:nth-child(2)', (content) =>
      Number(content.innerText)
    )
    .catch((error) => processError(error));
  return medal;
...
const getPoints = async (item) => {
  const points = await item
    .$eval('div:nth-child(2) > div:nth-child(6)', (content) => Number(content.innerText.replace(/,/g, '')))
    .catch((error) => processError(error));
  return points;
};
